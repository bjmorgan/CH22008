...
$$\frac{d[\mathrm{A}]}{dt} = -(k_1 + k_2)[\mathrm{A}]$$
...
$$\frac{d[\mathrm{B}]}{dt} = k_1[\mathrm{A}] = k_1[\mathrm{A}]_0\mathrm{e}^{-(k_1+k_2)t}$$
...
1. The formation of both products B and C is controlled by the same overall rate constant (k~1~+k~2~)
