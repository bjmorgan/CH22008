...
We can then use the standard relationship $\Delta G^\ddagger = \Delta H^\ddagger - T \Delta S^\ddagger$ to separate $K^\ddagger$ into enthalpic and entropic terms:

\begin{equation}
K^{\ddagger} = \mathrm{e}^{-\Delta G^{\ddagger}/RT} = \mathrm{e}^{-\Delta H^{\ddagger}/RT}\mathrm{e}^{\Delta S^{\ddagger}/R}
