...
### Dimensional Consistency in Eyring Plots
...
An alternative, dimensionally consistent, approach is to plot $\ln(kh/k_\mathrm{B}T)$ versus $1/T$, where $kh/k_\mathrm{B}T$ is dimensionless. Starting from the Eyring equation and rearranging:

\begin{equation}
\ln\left(\frac{kh}{k_\mathrm{B}T}\right) = \frac{\Delta S^{\ddagger}}{R} - \frac{\Delta H^{\ddagger}}{R}\frac{1}{T}
\end{equation}
...
\begin{equation}
\ln\left(\frac{kh}{k_\mathrm{B}T}\right) = \ln\left(\frac{k}{T}\frac{h}{k_\mathrm{B}}\right) =  \ln\left(\frac{k}{T}\right) + \ln\left(\frac{h}{k_\mathrm{B}}\right) = \ln\left(\frac{k}{T}\right) - \ln\left(\frac{k_\mathrm{B}}{h}\right)
\end{equation}

Both forms give identical slopes (and hence identical $\Delta H^\ddagger$ values). The intercepts differ by exactly $\ln(k_\mathrm{B}/h)$ = 23.76.^[Evaluating $\ln(k_\mathrm{B}/h)$ using $k_\mathrm{B} = 1.381 \times 10^{-23}~\mathrm{J~K^{-1}}$ and $h = 6.626 \times 10^{-34}~\mathrm{J~s}$ gives 23.76. Strictly speaking, both $\ln(k/T)$ and $\ln(k_\mathrm{B}/h)$ are mathematically ill-defined because logarithms require dimensionless arguments. In practice, these expressions involve implicit division by reference quantities with matching units. When we subtract, these implicit references cancel, giving the dimensionless result $\ln(kh/k_\mathrm{B}T)$. The conventional procedure thus produces correct values for $\Delta S^\ddagger$.] For the conventional form, the intercept is $\ln(k_\mathrm{B}/h) + \Delta S^\ddagger/R$, which is why we must subtract $\ln(k_\mathrm{B}/h)$ when extracting $\Delta S^\ddagger$.

In practice, the conventional form $\ln(k/T)$ versus $1/T$ is widely used in textbooks (and exam questions), both for historical reasons, and because it is computationally simpler and easier to use for manual calculations. This conventional plot is what is typically referred to as an "Eyring plot" in the literature. 
